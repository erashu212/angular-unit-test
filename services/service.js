...
UserService.constant('myConfig',
	{

		baseURL: 'http://cortexapi.ddns.net:8080'

	})
UserService.config(function($httpProvider) {
	$httpProvider.interceptors.push('TokenInterceptor');
});

// Login services
UserService.factory('LoginService', ["$http", "$window", "myConfig", function($http, $window, $myConfig, Base64) {
    ...
    return {
        logIn: function(username, password) {
            ...
            return $http.post($myConfig.baseURL + '/api/authenticate', { 'username' : username, 'password' : password}).success(function(data){
                });
        },

        register: function(rname, email, username, password, skills, IsLecturer) {
            return $http.post($myConfig.baseURL + '/api/addNewPerson', {name: rname, email: email, username: username, password: password, skills: skills, lecturer: IsLecturer}).success(function(data){
                });
        },
        verify: function() {
